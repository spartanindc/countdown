...
$(document).ready(function() {
  if ($("#countdowns-length").length) {
    ...
    for (var i = 0; i < countdownLength; i++) {
      if (
        new Date(
          $("#clock-" + i).attr("data-time-left") + "T00:00:00"
        ).getTime() /
          1000 -
          new Date().getTime() / 1000 >
        0
      ) {
        var clock = $("#clock-" + i).FlipClock(
          new Date(
            $("#clock-" + i).attr("data-time-left") + "T00:00:00"
          ).getTime() /
            1000 -
        ...
        $("#clock-" + i).html(
          '<p class="finished">Countdown has finished!</p>'
        );
      }
    }
  }
...
// Set up flaps ////////////////////////////////////////////
speed = 0.18; // seconds
beginStr = "COUNTDOWN".toUpperCase().split("");
endStr = "DDMMYYYY".toUpperCase().split("");
// A-Z, 0-9, spaces only

amountOfFlaps =
  beginStr.length >= endStr.length ? beginStr.length : endStr.length;

div = document.querySelector(".flipper");
html = "";
